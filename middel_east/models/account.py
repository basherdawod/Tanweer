...
class AccountMove(models.Model):
    ...
    middel_id = fields.Many2one('middel.east')
    ...
    def action_view_source_middel_booking(self):
        ...
        result = self.env['ir.actions.act_window']._for_xml_id('middel_east.action_middel_booking')
        if len(source_orders) > 1:
            result['domain'] = [('id', 'in', source_orders.ids)]
        elif len(source_orders) == 1:
            result['views'] = [(self.env.ref('middel_east.tailor_middel_form_view', False).id, 'form')]
